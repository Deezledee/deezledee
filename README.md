# Hi, I'm Diana (Dee) 👋

**Backend & Data Engineer · AI Applications · Automation**  
Berlin, Germany 🇩🇪

---

## 🧭 About Me

I'm a Berlin-based developer who genuinely loves tech and building useful products. My focus is backend and data-driven development, and I'm currently deepening my expertise in Data Science, MLOps and AI Engineering through hands-on practice.

- 🔭 Currently building **Project Gaia** — a software lifecycle & governance platform replacing spreadsheet-heavy workflows with an orchestrated multi-service system (Strapi · Docker · PostgreSQL · Kubernetes-native)
- 🤖 Exploring **AI agents**, RAG workflows and LLM-based applications
- 🗄️ Experienced in ERP integrations, data migration, SQL reporting and workflow automation
- 🥋 Shotokan Karate **3rd Dan** — discipline, focus and continuous improvement, on and off the mat (the greatest passion which I integrate in some of my projects too)

---

## 🛠️ Tech Stack

**Languages**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=flat&logo=c-sharp&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)

**Backend & APIs**  
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![Express](https://img.shields.io/badge/Express.js-000000?style=flat&logo=express&logoColor=white)
![Strapi](https://img.shields.io/badge/Strapi-2F2D74?style=flat&logo=strapi&logoColor=white)
![GraphQL](https://img.shields.io/badge/GraphQL-E10098?style=flat&logo=graphql&logoColor=white)
![REST APIs](https://img.shields.io/badge/REST_APIs-FF6C37?style=flat&logo=postman&logoColor=white)

**Data & AI**  
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=langchain&logoColor=white)

**Cloud & Infrastructure**  
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

**Frontend**  
![React](https://img.shields.io/badge/React-61DAFB?style=flat&logo=react&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)

---

## 🚀 Projects

### 🤖 [AI Sales Assistant Agent](https://sales-assistant-agent.onrender.com/chat)
AI assistant built with Python and LangChain following a **ReAct-style architecture**. Uses tool selection (CRM lookup, pricing retrieval and a knowledge-base RAG tool) to answer customer, pricing and policy questions dynamically.

[![Chat Demo](https://img.shields.io/badge/Chat_Demo-Live-brightgreen?style=flat)](https://sales-assistant-agent.onrender.com/chat)
[![API Docs](https://img.shields.io/badge/API_Docs-Swagger-85EA2D?style=flat&logo=swagger&logoColor=black)](https://sales-assistant-agent.onrender.com/docs)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/Deezledee/sales-assistant-agent)

---

### 🦠 [COVID-19 Detection from Chest X-Ray Images](https://dianadeekalstein.com/docs/COVID-19%20Detection%20from%20Chest%20X-Ray%20Images%20Project.pdf)
Collaborative data science project using deep learning (CNNs, Transfer Learning) to detect COVID-19 in chest X-rays. Includes **Grad-CAM interpretability** visualizations in a Streamlit app. *(Streamlit app ran locally — not hosted due to bootcamp time constraints.)*

[![Presentation](https://img.shields.io/badge/Presentation-PDF-red?style=flat&logo=adobeacrobatreader&logoColor=white)](https://dianadeekalstein.com/docs/COVID-19%20Detection%20from%20Chest%20X-Ray%20Images%20Project.pdf)

---

### 🌱 Project Gaia *(in development)*
Software lifecycle and governance platform designed to replace fragmented spreadsheet-based processes with a usable, integrated system. Gaia combines a Strapi-powered registry with service orchestration and integrations to external governance solutions, enabling connected workflows across tools instead of isolated data silos.

[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/pj-gaia)

---

### 🥋 [Karate Portal](https://getdojo.netlify.app/)
Full-stack karate portal where users can sign up, log in, manage their karateka profile, browse techniques and explore martial-arts personalities — full CRUD functionality.

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat)](https://getdojo.netlify.app/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/Deezledee/getDOJO)

---

### 🎮 [P5 Browser Game](https://my-family-guy-game.netlify.app/)
Interactive browser game inspired by one of my favorite TV shows, Family Guy *(not affiliated; the project is for learning purposes as part of a web-dev bootcamp with respect for intellectual property)*. Built with P5.js, showcasing game logic and interactive front-end design.

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat)](https://my-family-guy-game.netlify.app/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/Deezledee/Family-Guy-Project)

---

### 🌤️ [Weather Prediction App](https://adoring-kowalevski-9714eb.netlify.app/)
One of my first projects from the SheCodes workshop that deepened my understanding of API integrations, responsive design and JavaScript fundamentals. Built with live weather data and an intuitive UI, this project was instrumental in my journey into advanced web development phases.

[![Live](https://img.shields.io/badge/Live-Demo-brightgreen?style=flat)](https://adoring-kowalevski-9714eb.netlify.app/)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/Deezledee/Weather-App-Diana-K)

---

## 🎓 Education

| Year | Program | Institution |
|------|---------|-------------|
| 2025–2026 | Data Science & Machine Learning Bootcamp | Université Paris 1 Panthéon-Sorbonne · Liora |
| 2023 | Full-Stack Web Development Bootcamp | Ironhack, Berlin |
| 2021–2022 | Web Development Coding Workshop | SheCodes (Remote) |
| 2013–2016 | B.Sc. Economics & Business Management (Finance) | Ruppin Academic Centre, Israel |

---

## 🌍 Languages

🇬🇧 English (C1)  &nbsp;·&nbsp;  🇩🇪 German (C2)  &nbsp;·&nbsp;  🇮🇱 Hebrew (Native)

---

## 📫 Get in Touch

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/diana-kalstein)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=flat&logo=vercel&logoColor=white)](https://dianadeekalstein.com)

---

<p align="center"><i>"Fall seven times, stand up eight."</i></p>
